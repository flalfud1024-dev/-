...
> ## 본 시스템의 연구 내 위치 — **보조 baseline**
>
> 본 학위논문의 **메인 연구는 3개 중국어 번역본 간 비교**(연구 1·2·3)이며, Yes24 한국 독자 데이터는 그 비교를 해석하기 위한 **보조 기준선(auxiliary baseline)** 역할을 한다.
>
> **활용 방식**:
> - 원천 언어(한국어) 독자가 작품을 어떻게 수용하는지를 reference point로 제공
> - 중국어 번역본 독자반응의 특이점을 한국 독자반응 대비로 해석
> - 메인 통계 검증 결과의 외적 타당성 보강
>
> 따라서 본 시스템의 데이터 규모·정밀도는 **메인 연구를 보조하는 수준**이면 충분하며, 한·중 직접 등가 비교가 목적이 아니다.
...
### 2.1 연구 맥락 (보조 baseline)
| 대상 | Yes24 상품 ID | 설명 |
|------|---------------|------|
| 한강, 『채식주의자』 (2007 창비 초판 또는 최신판) | `108422348` | 자료 §5에서 확인 |

본 baseline 데이터는 메인 연구 1·2·3의 해석 단계에서 활용:
- 연구 1(섹슈얼리티 하이재킹): 한국 독자가 성적 장면을 어떻게 해석하는지 → 중국 검열본 독자반응의 결손 진단
- 연구 2(정신질환 프레이밍): 한국 독자의 영혜 인물 해석 → 중국 표제어 프레이밍의 침투 효과 측정 기준선
- 연구 3(전면 비교): 한국 독자의 전반적 주제 분포 → 두 중국어 번역본 독자반응 차이의 맥락화

> ⚠️ Yes24는 동일 도서의 여러 판본(초판/리커버/세트)이 별도 상품으로 등록되는 경향. 본 분석에 어느 판본을 포함할지 사전 결정 필요.

### 2.2 목적
- **G1**: 채식주의자 한국어 원본 리뷰 N ≥ 100건 (보조 baseline 용도, 통계 검증용 최소 표본)
...
- v1.1 (2026-05-10): 역할 명확화 — Yes24는 메인이 아닌 **보조 baseline**. 목표 N 200→100 하향 조정.
